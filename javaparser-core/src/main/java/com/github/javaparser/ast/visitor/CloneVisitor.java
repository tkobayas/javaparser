...
public class CloneVisitor implements GenericVisitor<Visitable, Object> {
    ...
    @Override
    public Visitable visit(final InlineCastExpr n, final Object arg) {
        Expression expression = cloneNode(n.getExpression(), arg);
        Type type = cloneNode(n.getType(), arg);
        Comment comment = cloneNode(n.getComment(), arg);
        InlineCastExpr r = new InlineCastExpr(n.getTokenRange().orElse(null), type, expression);
        r.setComment(comment);
        n.getOrphanComments().stream().map(Comment::clone).forEach(r::addOrphanComment);
        copyData(n, r);
        return r;
    ...
    @Override
    public Visitable visit(final HalfBinaryExpr n, final Object arg) {
        Expression right = cloneNode(n.getRight(), arg);
        Comment comment = cloneNode(n.getComment(), arg);
